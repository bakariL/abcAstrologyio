...
add_action('wp_ajax_CREATE_FREE_REPORT_USER', 'addUserForSaturnReport'); 
add_action('wp_ajax_nopriv_CREATE_FREE_REPORT_USER', 'addUserForSaturnReport' );
add_action('wp_ajax_SELECT_SATURN_REPORT', 'selectSaturnReport'); 
add_action('wp_ajax_VIEW_REPORT_SIGNUP', 'viewReportSignup' );
add_action('wp_ajax_nopriv_VIEW_REPORT_SIGNUP', 'viewReportSignup' );
add_action('um_post_registration_global_hook', 'welcomeEmail', 10, 2); 

function welcomeEmail($user_id, $args){
    
    
    
    $user_description = get_user_meta($user_id, 'description', true);
    if(strcmp($user_description, 'CREATED_FOR_FREE_REPORT') == 0){
        return;
    }
    ...
    $subject = "Welcome to ABC Astrology!";   
    $headers = "MIME-Version: 1.0" . "\r\n";
    $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
    $headers .= 'From: '.$abcastrology_from_email."\r\n" .'Reply-To: '.$abcastrology_from_email. "\r\n" .'X-Mailer: PHP/' . phpversion();
    ...
    <html>
        ...
        <body style="font-family: sans-serif; text-align: center; background: #fff4e0; padding:40px 0px;">
            <div style="display: inline-block; padding: 20px; background: #fff; border-radius: 10px;">
                ...
                <h1> Welcome '.$firstname.', <span style="font-weight: 300;"> to ABC Astrology!</span></h1>
    ...
    mail($user_email, $subject, $mailMessageUser, $headers);
}

function addUserForSaturnReport(){
    ...
    $gender = $_POST['gender'];
    $reportIdentifier = wp_generate_password(16, false);
    ...
    if(!$user_id){
        ...
        $userdata = array(
            ...
            'user_email'   =>  $email,
            'user_pass' => NULL,
            'description' => "CREATED_FOR_FREE_REPORT"
        );
        $user_id = wp_insert_user( $userdata ) ;
    }
        
        add_user_meta( $user_id, 'abc_report_identifier', $reportIdentifier);
        add_user_meta( $user_id, 'abc_report_'.$reportIdentifier.'_firstname', $firstname);
        add_user_meta( $user_id, 'abc_report_'.$reportIdentifier.'_lastname', $lastname);
        add_user_meta( $user_id, 'abc_report_'.$reportIdentifier.'_birthday', $birthday);
        add_user_meta( $user_id, 'abc_report_'.$reportIdentifier.'_gender', $gender);
    

    //On success
    if ( ! is_wp_error( $user_id ) ) {
        die('{"ID":"'.$user_id.'", "reportID":"'.$reportIdentifier.'"}');
...
function selectSaturnReport(){
    ...
    if(get_user_meta($user_id, 'abc_report_'.$reportIdentifier.'_report', $single) == '')
        add_user_meta( $user_id, 'abc_report_'.$reportIdentifier.'_report', $reportID);
    else
        update_user_meta( $user_id, 'abc_report_'.$reportIdentifier.'_report', $reportID);
...
add_action( 'after_setup_theme', 'mrtailor_set_product_columns_and_rows' );
function mrtailor_set_product_columns_and_rows() {

	if( get_option('mrtailor_product_columns_updated', false) == 0 ){

		$theme_options = get_option('mr_tailor_theme_options', false);
		if( $theme_options ) {

			$products_per_column = $theme_options['products_per_column'];

			if( isset( $products_per_column ) && $products_per_column != "" ) {
				update_option( 'woocommerce_catalog_columns', $products_per_column );
				update_option('mrtailor_product_columns_updated', true);
			}
		}
	}

	if( get_option('mrtailor_product_rows_updated', false) == 0 ){

		$theme_options = get_option('mr_tailor_theme_options', false);
		if( $theme_options ) {

			$products_per_page = $theme_options['products_per_page'];
			...
			if( isset( $products_per_page ) && $products_per_page != "" ) {
				...
				update_option('mrtailor_product_rows_updated', true);
			}
		}
	}
